  classdef TLOutputCamExpose < hw.TLOutput
  %HW.TLOutputCamExpose
  % To output a locked signal which doesn't ramp (camera exposure)
  %  %
  %
  % 2019-09 DS created from HW.TLOutputRampIllumination 
  
  properties
    DaqDeviceID % The name of the DAQ device ID, e.g. 'Dev1', see DAQ.GETDEVICES
    DaqVendor = 'ni' % Name of the DAQ vendor   
    exposureOutputChannelID; % camera exposure output channel (must be analog, could not work out digital)
    triggerChannelID; % the channel to trigger output (acqlive)
    framerate; % Hz
...
%     end
  ...
  function obj = TLOutputCamExpose()
    ...
    function init(obj, ~)
      ...
      queueSeconds = 7; %obj.framerate/gcd(obj.framerate,lightRate);
      ...
      %exposureSamplesFull = double(exposureSamplesFull > 0); %binalize
      ...
      %         obj.Session.addDigitalChannel(obj.DaqDeviceID, obj.exposureOutputChannelID, 'OutputOnly'); %as in TLOutputAcqLive
    ...
    function start(obj, ~)
     ...
     %     src.queueOutputData([obj.illuminationSamples,obj.exposureSamples]));      
      ...
      % Start output NG within this line!
    ...
    function s = toStr(obj)
      ...
      %
      % See Also INIT
        s = sprintf('%s: %g Hz framerate, %g ms exposure', obj.Name, ...
            obj.framerate, obj.lightExposeTime);
    end
  end
