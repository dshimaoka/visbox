classdef Timeline < handle
    ...
    methods
        ...
        function start(obj, expRef)
            ...
            if obj.WriteBufferToDisk
                ...
                mkdir(fileparts(localPath)); %create new local folder %23/7/20
                %end
        ...
        function stop(obj)
            ...
            %  write hardware info to a JSON file for compatibility with
            %  database 15/7/19 commented out DS
    ...
    methods (Access = private)
        ...
        function livePlot(obj, data)
            ...
            if isempty(obj.Axes)
                f = figure('Units', 'Normalized', 'Position', [0 .3 1 .7]); % create a figure for plotting aquired data
            ...
            resampleFac = 5; %16/6/20
            showDuration = 10; %[s] 16/5/20
            
            data = data(1:resampleFac:end,:); %down sampling to reduce burden. 
            %this helps to reduce the following error:
            %             Warning: clocking pulse not detected - probably lagging
            ...
            if isempty(traces)
                Fs = obj.DaqSampleRate/resampleFac;
                plot((1:Fs*showDuration)/Fs, zeros(Fs*showDuration, length(names))+repmat(offsets, Fs*showDuration, 1));
                traces = get(obj.Axes, 'Children');
            ...
            scales(find(contains(meas,'Position'))) = 0.1*scales(find(contains(meas,'Position'))); %23/7/20
