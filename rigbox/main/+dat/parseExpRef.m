function [subjectRef, expDate, expSeries, expSequence] = parseExpRef(ref)
...
% 2013-03 CB created
% 2020-03 DS expdate having two columns  [datenum seriesSuffix]

if isempty(ref) 
  subjectRef = cell(size(ref));
  expDate = zeros(size(ref));
  expSeries = zeros(size(ref)); %31/3/20
  expSequence = zeros(size(ref));
elseif iscell(ref) %NOT YET
  ...
  expSeries = reshape(cellstr2double({parsed.series}), size(ref)); %31/3/20
  ...
  expSeries = cellstr2double(parsed.series); %31/3/20
